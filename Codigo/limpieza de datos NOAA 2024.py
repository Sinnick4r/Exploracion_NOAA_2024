...
def convert_damage(value: str) -> float:
    """
...
def select_file(file_type: str) -> str:
    ...
    devuelve la ruta completa del archivo o una cadena vacía si se cancela.
...
def load_csv_data(file_path: str, file_description: str) -> pd.DataFrame:
    """
...
def handle_null_columns(df: pd.DataFrame, threshold: float = 0.5) -> pd.DataFrame:
    """
...
def correct_out_of_range_values(df: pd.DataFrame) -> pd.DataFrame:
    """
    Corrige valores fuera de rango en columnas específicas.

    """
    if 'FATALITY_AGE' in df.columns:
        df['FATALITY_AGE'] = df['FATALITY_AGE'].apply(lambda x: x if 0 <= x <= 120 else -1)
...
def convert_to_datetime(df: pd.DataFrame, year_month_col: str = None, day_col: str = None, time_col: str = None, output_col_prefix: str = None, date_format: str = '%Y%m', output_format: str = None) -> pd.DataFrame:
    """
...
def clean_detalles(detalles_df: pd.DataFrame) -> pd.DataFrame:
    """
    Limpieza específica para el DataFrame de detalles.
    """
    detalles_df['DAMAGE_PROPERTY'] = detalles_df['DAMAGE_PROPERTY'].fillna('0').apply(convert_damage)
...
def clean_fatalidades(fatalidades_df: pd.DataFrame) -> pd.DataFrame:
    """
...
def clean_lugares(lugares_df: pd.DataFrame) -> pd.DataFrame:
    """
...
def data_quality_report(df: pd.DataFrame) -> None:
    """
    Genera un reporte de lo trabajado
...
def main():
    ...
    detalles_path = select_file("detalles")
    fatalidades_path = select_file("fatalidades")
    lugares_path = select_file("lugares")

    detalles_df = load_csv_data(detalles_path, "detalles")
    fatalidades_df = load_csv_data(fatalidades_path, "fatalidades")
    lugares_df = load_csv_data(lugares_path, "lugares")
    ...
    detalles_df = clean_detalles(detalles_df)
    fatalidades_df = clean_fatalidades(fatalidades_df)
    lugares_df = clean_lugares(lugares_df)

    detalles_df = handle_null_columns(detalles_df)
    detalles_df = correct_out_of_range_values(detalles_df)

    master_df = detalles_df.merge(fatalidades_df, on='EVENT_ID', how='left').merge(lugares_df, on='EVENT_ID', how='left')

    master_df = convert_to_datetime(master_df, year_month_col='BEGIN_YEARMONTH', day_col='BEGIN_DAY', time_col='BEGIN_TIME', output_col_prefix='BEGIN')
    master_df = convert_to_datetime(master_df, year_month_col='END_YEARMONTH', day_col='END_DAY', time_col='END_TIME', output_col_prefix='END')

    data_quality_report(master_df)

    output_file = 'master_cleaned_storm_events.csv'
